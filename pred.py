...
def main(base_path, pred_out_path, pred_func, set_name=None):
    ...
    K_list = utils.json_load(os.path.join(base_path, "%s_K.json" % set_name))
    scale_list = utils.json_load(os.path.join(base_path, "%s_scale.json" % set_name))

    # iterate over the dataset once
    for idx in tqdm(range(utils.db_size(set_name))):
        if idx >= utils.db_size(set_name):
            break

        # load input image
        img = utils.read_img(idx, base_path, set_name)
