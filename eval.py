...
def _search_pred_file(pred_path, pred_file_name):
    ...
    files = [
        os.path.join(pred_path, x) for x in os.listdir(pred_path) if x.endswith(".json")
    ]
    if len(files) == 1:
...
def main(gt_path, pred_path, output_dir, pred_file_name=None, set_name=None):
    ...
    # init eval utils
    eval_xyz, eval_xyz_aligned = EvalMetrics(), EvalMetrics()
    eval_mesh_err, eval_mesh_err_aligned = EvalMetrics(num_kp=778), EvalMetrics(
        num_kp=778
    )
    ...
    xyz_al_mean3d, _, xyz_al_auc3d, pck_xyz_al, thresh_xyz_al = (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "output_dir", type=str, help="Path to where the eval result should be."
    )
    parser.add_argument(
        "--pred_file_name", type=str, default="pred.json", help="Name of the eval file."
    )
