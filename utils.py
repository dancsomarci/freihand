...
def plot_hand(
    ...
    if vis is None:
        vis = abs(np.ones_like(coords_hw[:, 0]) - 1.0) < 0.001
...
def load_db_annotation(base_path, set_name=None):
    ...
    return np.array(K_list), np.array(xyz_list)
...
class sample_version:
    ...
    auto = (
        "auto"  # auto colorization without sample points: automatic color hallucination
    )
...
def read_msk(idx, base_path):
    mask_path = os.path.join(base_path, "training", "mask", "%08d.jpg" % idx)
